...
def strengthen_sound_core(strengthen_level: int, entry: str)-> bool:
    """
    通用的声骸强化界面
    ...
    try:
        while True:
            result = automaticsimulation.recognize_char((1770, 250), (1830, 280))# 检测声骸等级
            # 1.检测声骸等级:等级存在>0,培养
            if result and (result:=result[0][0].replace("+", "")) and result.isdigit() and int(result) == 0:
            ...
            # ---------------------------------------------------------------强化圣遗物---------------------------------------------------------------------- #
            # .检测等级:等级=5/10,完成一次
            elif (result := automaticsimulation.recognize_char((180, 180), (239, 206))) and (result:=result[0][0].replace("+", ""))  and result.isdigit() and int(result) >= strengthen_level :
                # 12.检测词条:有指定词条,没有锁定,点击锁定
                if automaticsimulation.match_char((221, 324), (414, 522), entry) and not automaticsimulation.get_three_pixel_color(674, 129,"1A1A1A", 674,128, "1A1A1A",670, 135,"1A1A1A"):
                    automaticsimulation.key_down_times("C", 1, 50, 200)
                # 13.检测词条:没有指定词条,没有弃置,点击弃置
                elif automaticsimulation.recognize_char((221, 324),(414, 522)) and automaticsimulation.get_three_pixel_color(610, 129,"1A1A1A",623, 133,"1A1A1A",616, 122,"FFFFFF"):
                    automaticsimulation.key_down_times("Z", 1, 50, 200)
            ...
            # 16.检测声骸等级：不存在等级/等级>0
                    ...
                    and result.isdigit()
                    and int(result) > 0
                    or automaticsimulation.match_char((934, 510), (1052, 547), "暂无内容")
            ):
                print("无声骸升级")
                return True
    except Exception as e:
